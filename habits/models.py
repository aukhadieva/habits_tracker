...
class Habit(models.Model):
    ...
    behavior = models.CharField(max_length=200, verbose_name='Действие',
                                db_comment='действие, которое представляет собой привычка')
    time = models.TimeField(verbose_name='Время', db_comment='время, когда необходимо выполнять привычку')
    ...
    def __str__(self):
        return (f'Привычка {self.behavior} пользователя {self.owner} '
                f'(признак приятной привычки: {self.is_pleasant_habit})')
